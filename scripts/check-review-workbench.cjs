...
async function main() {
  ...
  assertIncludes(css, "@media (max-width: 620px)");
  assertIncludes(css, ".workbench-shell");
  assertIncludes(css, ".review {");
  assertIncludes(css, "grid-template-columns: 1fr auto 1fr");
  assertIncludes(css, ".field[data-decided=\"1\"] .decide {\n  display: none;");
  assertIncludes(css, ".reference-details summary");
  assertIncludes(css, "overflow-wrap: anywhere");
  assertIncludes(css, "--k-muted: var(--k-text-muted)");
  assertIncludes(css, "--k-radius: var(--k-radius-md)");
  ...
  assertIncludes(js, "setDecision");
  assertIncludes(js, "clearDecision");
  assertIncludes(js, "Use proposed");
  assertIncludes(js, "Keep current");
  assertIncludes(js, "Suggestion was wrong");
  assertIncludes(js, "Needs review");
  assertIncludes(js, "Producer feedback tags");
  assertIncludes(js, "Audit details");
