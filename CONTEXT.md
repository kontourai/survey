...
A flat producer provenance record that captures how an actor read a specific **Raw Source** locator for exactly one target **Claim**. An **Interpretation Record** carries a reading kind: the original `policy-standard` reading (how a **Policy Standard Raw Source** paragraph applies; absent kind means this, and it must anchor to a policy-standard source), a `gleaned` reading (what the anchored result source taught for the claim), or an `answerImpact` reading (how the result moved the inquiry answer the claim feeds: supported, narrowed, or accepted-risk). Every kind has an `appliesTo` edge to the claim and an `anchorsTo` edge to a known raw source; Survey validates those references and projects the trace as a Surface-compatible verification event plus typed claim metadata. Every reading is authored producer judgment and must present as such, distinct from machine-observed facts. Survey preserves the reading, actor, time, locator, and anchor metadata, but does not decide whether the reading is correct.
_Avoid_: Rejection reason, claim dependency, nested interpretation graph, treating the interpretation as the policy decision itself, presenting a reading as a machine-observed fact, loose free-text fields outside the record
