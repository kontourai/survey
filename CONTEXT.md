...
A flat producer provenance record that captures how an actor read a specific **Policy Standard Raw Source** locator for exactly one target **Claim**. An **Interpretation Record** has an `appliesTo` edge to the claim and an `anchorsTo` edge to the policy-standard raw source; Survey validates those references and projects the trace as a Surface-compatible verification event plus typed claim metadata. Survey preserves the reading, actor, time, rule locator, and anchor metadata, but does not decide whether the reading is correct.
_Avoid_: Rejection reason, claim dependency, nested interpretation graph, treating the interpretation as the policy decision itself
